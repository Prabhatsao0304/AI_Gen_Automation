...
export class CollectionsPage {
  constructor(page) {
    this.page = page;
    this.url = `${config.products['fmt-os'].baseUrl}/finance/collections`;
  ...
  async navigateToCollections() {
    ...
    await firstMatchingLocator(this.page, this._anyTabStrategies(), {
      context: 'Collections: tabs visible',
  ...
  async search(text) {
    await fillFirstMatching(this.page, this._searchStrategies(), text, {
      context: 'Collections: search input',
      logFallback: this._healLog,
      perTryTimeout: 6000,
      retryRecovery: async () => {
        await this.page.goto(this.url, { waitUntil: 'domcontentloaded' });
  ...
  async clickTabAndVerifyEmptyState(tabName, primaryText, supportingText) {
    try {
      await clickFirstMatching(this.page, this._tabStrategies(tabName), {
        context: `Collections: click tab "${tabName}"`,
        logFallback: this._healLog,
        perTryTimeout: 8000,
      });
    } catch {
      throw bugError('EMPTY_SCREEN_TAB_MISSING', `Collections: expected tab "${tabName}" to exist/be clickable.`);
    }
    ...
    try {
      ...
      throw bugError(
        ...
        `Collections "${tabName}": primary text missing/changed. Expected: "${primaryText}".`
      );
    }
    ...
    try {
      ...
      throw bugError(
        'EMPTY_SCREEN_COPY_MISSING',
        `Collections "${tabName}": supporting text missing/changed. Expected: "${supportingText}".`
      );
    }
  }
}
